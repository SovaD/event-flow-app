...
export default function Register({ setAuth }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Используем unwrap() для получения результата или выброса ошибки из Thunk
      await dispatch(registerUser(formData)).unwrap();
      
      // Если регистрация успешна, вызываем setAuth(true), 
      // если этот пропс всё ещё нужен вашему App.js (до полного перехода на Redux-маршрутизацию)
      if (setAuth) setAuth(true); 
    } catch (err) {
      // Ошибка уже записалась в Redux (state.auth.error) благодаря rejectWithValue,
      // поэтому здесь ничего дополнительно делать не нужно.
    }
  };

  const handleChange = (e, field) => {
    // Если пользователь начал печатать после ошибки, очищаем ошибку в сторе
