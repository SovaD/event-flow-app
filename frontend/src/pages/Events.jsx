...
const getCategoryStyle = (category) => {
  const styles = {
    Свадьба: { bg: "#ffe4e6", color: "#e11d48" },
    "День рождения": { bg: "#e0f2fe", color: "#0284c7" },
    Корпоратив: { bg: "#f3e8ff", color: "#9333ea" },
    Вечеринка: { bg: "#ffedd5", color: "#ea580c" },
    Другое: { bg: "#f1f5f9", color: "#475569" },
  };
...
export default function Events() {
  ...
  useEffect(() => {
    if (selectedEvent && events.length > 0) {
      ...
      if (
        updatedSelected &&
        JSON.stringify(updatedSelected) !== JSON.stringify(selectedEvent)
      ) {
  ...
  const handleAddOrEditEvent = async (e) => {
    ...
    try {
      if (editingEventId) {
        await axios.put(`${API_URL}/events/${editingEventId}`, newEvent, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
  ...
  const handleQuickAddContact = async (e) => {
    ...
    const token = localStorage.getItem("token") || "mock_token";
    try {
      const res = await axios.post(`${API_URL}/contacts/add`, newQuickContact, {
        headers: { Authorization: `Bearer ${token}` },
      });
