...
export default function PublicInvite() {
  ...
  const handleRSVP = async (rsvpStatus) => {
    try {
      // Используем unwrap для перехвата ошибок из thunk
      await dispatch(submitRsvp({ eventId, guestId, status: rsvpStatus })).unwrap();
    } catch (error) {
      alert("Ой, что-то пошло не так. Попробуйте еще раз.");
    }
  };

  if (status === 'loading') return <div style={{ textAlign: 'center', marginTop: '50px', fontFamily: 'sans-serif' }}>Загрузка приглашения...</div>;
  if (!data && status !== 'idle') return <div style={{ textAlign: 'center', marginTop: '50px', color: 'red', fontFamily: 'sans-serif' }}>Приглашение не найдено или ссылка устарела.</div>;
  if (!data) return null; // Для первоначального состояния idle

  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', background: '#fff', minHeight: '100vh', boxShadow: '0 0 20px rgba(0,0,0,0.1)', fontFamily: 'system-ui, sans-serif' }}>
      {/* Обложка */}
      <img src={data.event.photoUrl || placeholderImg} alt="Cover" style={{ width: '100%', height: '250px', objectFit: 'cover' }} />
      ...
      <div style={{ padding: '25px', textAlign: 'center' }}>
        ...
        {replied ? (
          ...
          <div>
            <p style={{ fontWeight: 'bold', marginBottom: '15px' }}>Сможете ли вы прийти?</p>
            <div style={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
              <button onClick={() => handleRSVP('Подтвержден')} style={{ border: 'none', cursor: 'pointer', background: '#ff4b82', color: 'white', padding: '15px', fontSize: '16px', borderRadius: '30px', fontWeight: 'bold', transition: 'opacity 0.2s' }}>
                ✅ Да, я обязательно буду!
              </button>
              <button onClick={() => handleRSVP('Отклонен')} style={{ border: 'none', cursor: 'pointer', background: '#f0f0f0', color: '#555', padding: '15px', fontSize: '16px', borderRadius: '30px', fontWeight: 'bold', transition: 'background 0.2s' }}>
                ❌ К сожалению, не смогу
              </button>
            </div>
          </div>
        ...
        <div style={{ marginTop: '20px', fontSize: '12px', color: '#aaa' }}>
          Текущий статус: <span style={{ fontWeight: 'bold' }}>{data.currentStatus || 'Ожидает'}</span>
        </div>
      </div>
    </div>
  );
